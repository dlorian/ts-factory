...
const getDstDateTime = (year, month) => {
    if ('march' === month || 3 === month) {
        ...
        return getDate(lastSunday.get('year'), lastSunday.get('month'), lastSunday.get('day'), 2, 0, 0);

    } else if ('october' === month || 10 === month) {
        const lastSunday = getLastSunday(DateTime.local(year, 10, 31, 0, 0, 0));
        return getDate(lastSunday.get('year'), lastSunday.get('month'), lastSunday.get('day'), 3, 0, 0);

    } else {
        throw new Error(`${month} is not supported. Must be either march or october`);
    }
};
